# banking_functions.py
# Banking API integration for Owl Bank function calling

import json
import logging
from typing import Dict, Any, Optional
from datetime import datetime, timezone
import asyncio

logger = logging.getLogger(__name__)

class BankingAPIClient:
    """Client for Owl Bank API integration with function calling support"""
    
    def __init__(self, base_url: str = "https://owl-bank-finserv-demo-1-1-8657.twil.io"):
        self.base_url = base_url
        self.session = None
    
    async def get_session(self):
        """Get or create aiohttp session for async requests"""
        if self.session is None:
            self.session = aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5.0))
        return self.session
    
    async def close_session(self):
        """Close aiohttp session"""
        if self.session:
            await self.session.close()
            self.session = None
    
    async def get_banking_data(self, customer_phone: str) -> Dict[str, Any]:
        """Get customer banking data including account balance and recent transactions"""
        try:
            session = await self.get_session()
            url = f"{self.base_url}/get-banking-data"
            
            payload = {"customer_phone": customer_phone}
            
            async with session.post(url, json=payload) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(f"[BANK] Retrieved banking data for {customer_phone}")
                    return {
                        "success": True,
                        "data": data,
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    }
                else:
                    error_text = await response.text()
                    logger.error(f"[BANK] API error {response.status}: {error_text}")
                    return {
                        "success": False,
                        "error": f"API error {response.status}",
                        "message": "Failed to retrieve banking data"
                    }
        except asyncio.TimeoutError:
            logger.error(f"[BANK] Timeout getting banking data for {customer_phone}")
            return {"success": False, "error": "timeout", "message": "Request timeout"}
        except Exception as e:
            logger.error(f"[BANK] Exception getting banking data: {e}")
            return {"success": False, "error": "exception", "message": str(e)}
    
    async def transfer_money(self, from_account: str, to_account: str, amount: float, description: str = "") -> Dict[str, Any]:
        """Perform money transfer between accounts"""
        try:
            session = await self.get_session()
            url = f"{self.base_url}/transfer-money"
            
            payload = {
                "from_account": from_account,
                "to_account": to_account,
                "amount": amount,
                "description": description,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
            
            async with session.post(url, json=payload) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(f"[BANK] Transfer completed: {amount} from {from_account} to {to_account}")
                    return {
                        "success": True,
                        "data": data,
                        "transfer_id": data.get("transfer_id"),
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    }
                else:
                    error_text = await response.text()
                    logger.error(f"[BANK] Transfer failed {response.status}: {error_text}")
                    return {
                        "success": False,
                        "error": f"Transfer failed {response.status}",
                        "message": "Transfer could not be completed"
                    }
        except asyncio.TimeoutError:
            logger.error(f"[BANK] Timeout on transfer: {amount} from {from_account} to {to_account}")
            return {"success": False, "error": "timeout", "message": "Transfer timeout"}
        except Exception as e:
            logger.error(f"[BANK] Exception on transfer: {e}")
            return {"success": False, "error": "exception", "message": str(e)}
    
    async def pix_transfer(self, pix_key: str, amount: float, description: str = "") -> Dict[str, Any]:
        """Perform PIX transfer using PIX key"""
        try:
            session = await self.get_session()
            url = f"{self.base_url}/pix-transfer-sync-handling"
            
            payload = {
                "pix_key": pix_key,
                "amount": amount,
                "description": description,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }
            
            async with session.post(url, json=payload) as response:
                if response.status == 200:
                    data = await response.json()
                    logger.info(f"[BANK] PIX transfer completed: {amount} to {pix_key}")
                    return {
                        "success": True,
                        "data": data,
                        "pix_id": data.get("pix_id"),
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    }
                else:
                    error_text = await response.text()
                    logger.error(f"[BANK] PIX transfer failed {response.status}: {error_text}")
                    return {
                        "success": False,
                        "error": f"PIX transfer failed {response.status}",
                        "message": "PIX transfer could not be completed"
                    }
        except asyncio.TimeoutError:
            logger.error(f"[BANK] Timeout on PIX transfer: {amount} to {pix_key}")
            return {"success": False, "error": "timeout", "message": "PIX transfer timeout"}
        except Exception as e:
            logger.error(f"[BANK] Exception on PIX transfer: {e}")
            return {"success": False, "error": "exception", "message": str(e)}

# OpenAI Function Definitions for banking operations
BANKING_FUNCTION_DEFINITIONS = [
    {
        "type": "function",
        "function": {
            "name": "get_account_balance",
            "description": "Get customer's current account balance and recent transactions",
            "parameters": {
                "type": "object",
                "properties": {
                    "customer_phone": {
                        "type": "string",
                        "description": "Customer's phone number in E.164 format (e.g., +5511999887766)"
                    }
                },
                "required": ["customer_phone"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "transfer_money",
            "description": "Transfer money between accounts",
            "parameters": {
                "type": "object",
                "properties": {
                    "to_account": {
                        "type": "string",
                        "description": "Recipient account number or identifier"
                    },
                    "amount": {
                        "type": "number",
                        "description": "Amount to transfer (in BRL)"
                    },
                    "description": {
                        "type": "string",
                        "description": "Transfer description or memo"
                    }
                },
                "required": ["to_account", "amount"]
            }
        }
    },
    {
        "type": "function",
        "function": {
            "name": "pix_transfer",
            "description": "Perform PIX transfer using PIX key (phone, email, CPF, or random key)",
            "parameters": {
                "type": "object",
                "properties": {
                    "pix_key": {
                        "type": "string",
                        "description": "PIX key (phone number, email, CPF, or random key)"
                    },
                    "amount": {
                        "type": "number",
                        "description": "Amount to transfer via PIX (in BRL)"
                    },
                    "description": {
                        "type": "string",
                        "description": "PIX transfer description"
                    }
                },
                "required": ["pix_key", "amount"]
            }
        }
    }
]

class BankingFunctionHandler:
    """Handler for banking function calls from OpenAI"""
    
    def __init__(self):
        self.api_client = BankingAPIClient()
    
    async def handle_function_call(self, function_name: str, arguments: Dict[str, Any], customer_phone: str = None) -> Dict[str, Any]:
        """Handle function call and return response"""
        try:
            if function_name == "get_account_balance":
                # Use provided customer_phone or from arguments
                phone = arguments.get("customer_phone") or customer_phone
                if not phone:
                    return {"error": "Customer phone number required"}
                return await self.api_client.get_banking_data(phone)
            
            elif function_name == "transfer_money":
                # For transfers, we need the customer's own account as from_account
                if not customer_phone:
                    return {"error": "Customer phone number required for transfers"}
                
                return await self.api_client.transfer_money(
                    from_account=customer_phone,  # Use customer phone as account identifier
                    to_account=arguments["to_account"],
                    amount=arguments["amount"],
                    description=arguments.get("description", "")
                )
            
            elif function_name == "pix_transfer":
                return await self.api_client.pix_transfer(
                    pix_key=arguments["pix_key"],
                    amount=arguments["amount"],
                    description=arguments.get("description", "")
                )
            
            else:
                return {"error": f"Unknown function: {function_name}"}
        
        except Exception as e:
            logger.error(f"[BANK] Function call error: {e}")
            return {"error": "Function call failed", "message": str(e)}
    
    async def close(self):
        """Close API client session"""
        await self.api_client.close_session()
